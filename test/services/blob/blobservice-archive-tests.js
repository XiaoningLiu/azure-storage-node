...
describe('BlobArchive', function () {
  describe('Archive tests for block blobs in a blob storage account with LRS', function () {
    ...
    runBlockBlobCase('setBlobTier should work setting tier to hot for a block blob without tier', function (done) {
      blobService.setBlobTier(containerName, blobName, blobutil.BlockBlobTier.HOT, function (err, resp) {
        assert.equal(err, null);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(err, null);
          assert.equal(properties.accessTier, blobutil.BlockBlobTier.HOT);

          blobService.listBlobsSegmented(containerName, null, function (err, results, resp) {
            assert.equal(err, null);
            assert.equal(results.entries.length, 1);
            assert.equal(results.entries[0].accessTier, blobutil.BlockBlobTier.HOT);
            done();
          });
        });
      });
    ...
    runBlockBlobCase('setBlobTier should work setting tier to cool for a block blob without tier', function (done) {
      blobService.setBlobTier(containerName, blobName, blobutil.BlockBlobTier.COOL, function (err, resp) {
        assert.equal(err, null);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(err, null);
          assert.equal(properties.accessTier, blobutil.BlockBlobTier.COOL);

          blobService.listBlobsSegmentedWithPrefix(containerName, '', null, function (err, results, resp) {
            assert.equal(err, null);
            assert.equal(results.entries.length, 1);
            assert.equal(results.entries[0].accessTier, blobutil.BlockBlobTier.COOL);
            done();
          });
        });
      });
    ...
    runBlockBlobCase('setBlobTier should work setting tier to archive for a block blob without tier', function (done) {
      blobService.setBlobTier(containerName, blobName, blobutil.BlockBlobTier.ARCHIVE, function (err, resp) {
        assert.equal(err, null);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(err, null);
          assert.equal(properties.accessTier, blobutil.BlockBlobTier.ARCHIVE);

          blobService.listBlobsSegmented(containerName, null, function (err, results, resp) {
            assert.equal(err, null);
            assert.equal(results.entries.length, 1);
            assert.equal(results.entries[0].accessTier, blobutil.BlockBlobTier.ARCHIVE);
            done();
          });
        });
      });
    ...
    runBlockBlobCase('setBlobTier should not work setting page block tiers for a block blob', function (done) {
      blobService.setBlobTier(containerName, blobName, blobutil.PageBlobTier.P10, function (err, resp) {
        assert.notEqual(err, null);
        done();
      });
    });
  ...
  describe('Archive tests for page blobs in a premium storage account', function () {
    ...
    runPageBlobCase('setBlobTier should work setting higher tier for a page blob but not for lower tier', function (done) {
      blobService.setBlobTier(containerName, blobName, blobutil.PageBlobTier.P4, function (err, resp) {
        assert.equal(err, null);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(err, null);
          assert.equal(properties.accessTier, blobutil.PageBlobTier.P4);

          blobService.listBlobsSegmented(containerName, null, function (err, results, resp) {
            assert.equal(err, null);
            assert.equal(results.entries.length, 1);
            assert.equal(results.entries[0].accessTier, blobutil.PageBlobTier.P4);

            blobService.setBlobTier(containerName, blobName, blobutil.PageBlobTier.P6, function (err, resp) {
              assert.equal(err, null);

              blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
                assert.equal(true, (properties.accessTier === blobutil.PageBlobTier.P6));

                blobService.setBlobTier(containerName, blobName, blobutil.PageBlobTier.P4, function (err, resp) {
                  assert.notEqual(err, null);

                  blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
                    assert.equal(properties.accessTier, blobutil.PageBlobTier.P6);
                    done();
                  });
                });
              });
    ...
    runPageBlobCase('createPageBlob should work with setting page blob tier', function (done) {
      blobName += 'temp';
      blobService.createPageBlob(containerName, blobName, 0, { pageBlobTier: blobutil.PageBlobTier.P10 }, function (uploadError, uploadResponse) {
        assert.equal(uploadError, null);
        assert.ok(uploadResponse.isSuccessful);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(true, (properties.accessTier === blobutil.PageBlobTier.P10));
          done();
        });
      });
    });
    ...
    runPageBlobCase('createPageBlob should not work with setting small tier for large size blob', function (done) {
      blobName += 'temp';
      blobService.createPageBlob(containerName, blobName, 1024 * 1024 * 1024 * 512, { pageBlobTier: blobutil.PageBlobTier.P4 }, function (uploadError, uploadResponse) {
    ...
    runPageBlobCase('createPageBlob should not work with setting block blob tier for page blob', function (done) {
      blobName += 'temp';
      blobService.createPageBlob(containerName, blobName, 0, { pageBlobTier: blobutil.BlockBlobTier.HOT }, function (uploadError, uploadResponse) {
        assert.notEqual(uploadError, null);
        done();
      });
    });
    ...
    runPageBlobCase('createPageBlobFromLocalFile should work with setting page blob tier', function (done) {
      ...
      blobService.createPageBlobFromLocalFile(containerName, blobName, localFileName, { pageBlobTier: blobutil.PageBlobTier.P10 }, function (uploadError, result, uploadResponse) {
        assert.equal(uploadError, null);
        assert.ok(uploadResponse.isSuccessful);
    ...
    runPageBlobCase('createPageBlobFromStream should work with setting page blob tier', function (done) {
      ...
      blobService.createPageBlobFromStream(containerName, blobName, readStream, 1024 * 1024 * 4, { pageBlobTier: blobutil.PageBlobTier.P20 }, function (uploadError, result, uploadResponse) {
        assert.equal(uploadError, null);
        assert.ok(uploadResponse.isSuccessful);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(true, (properties.accessTier === blobutil.PageBlobTier.P20));
          fs.unlinkSync(localFileName);
          done();
        });
      });
    ...
    runPageBlobCase('createWriteStreamToNewPageBlob should work with setting page blob tier', function (done) {
      ...
      var writeStream = blobService.createWriteStreamToNewPageBlob(containerName, blobName, 1024 * 1024 * 4, { pageBlobTier: blobutil.PageBlobTier.P30 }, function (uploadError, result, uploadResponse) {
        assert.equal(uploadError, null);
        assert.ok(uploadResponse.isSuccessful);

        blobService.getBlobProperties(containerName, blobName, function (err, properties, resp) {
          assert.equal(true, (properties.accessTier === blobutil.PageBlobTier.P30));
          fs.unlinkSync(localFileName);
          done();
        });
      });
    ...
    runPageBlobCase('copyBlob should work with setting tier for page blob', function (done) {
      ...
      blobService.startCopyBlob(sourceBlobUri, containerName, copiedBlobName, { pageBlobTier: blobutil.PageBlobTier.P20 }, function (err, result, response) {
        setTimeout(function () {
