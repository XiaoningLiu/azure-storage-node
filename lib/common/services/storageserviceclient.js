...
StorageServiceClient.prototype._buildRequestOptions = function (webResource, body, options, callback) {
  ...
  // Browsers cache GET/HEAD requests and add conditional headers such as 'IF_MODIFIED_SINCE' after the 'Authorization header' calculation, 
  // which may get a 403 authorization error. So add timestamp to GET/HEAD request URLs thus avoid the browser cache. 
  if ((typeof window !== 'undefined') && (webResource.method === Constants.HttpConstants.HttpVerbs.GET || webResource.method === Constants.HttpConstants.HttpVerbs.HEAD)) {
  ...
  this.storageCredentials.signRequest(webResource, function (error) {
    ...
    if (!error) {
      ...
      requestOptions = {
        ...
        // Disable Fetch API because Edge may crash when sending requests with fetch()
        // The mode option will be finally passed to the browserify module 'stream-http'
        mode: 'disable-fetch'
      };
