...
exports.blobTierNameIsValid = function (blobTier, callback) {
  ...
  if (!_.chain(_.union(_.values(BlobUtilities.BlockBlobTier), _.values(BlobUtilities.PageBlobTier)))
    .map(function (val) { return val.toString().toUpperCase(); })
    .contains(blobTier.toString().toUpperCase())
    .value()) {
    return fail(new SyntaxError('Blob tier is incorrect. Refer to BlobUtilities.BlockBlobTier or BlobUtilities.PageBlobTier for possible values.'));
  }
