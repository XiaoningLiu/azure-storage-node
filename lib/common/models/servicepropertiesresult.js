...
function serializeDeleteRetentionPolicy(doc, policy) {
  if (policy !== null) {
    ...
    if (typeof policy.RetainedVersionsPerBlob !== 'undefined') {
      doc = doc.ele(ServicePropertiesConstants.DEFAULT_RETAINED_VERSIONS_PER_BLOB_ELEMENT)
        .txt(policy.RetainedVersionsPerBlob)
        .up();
    } else if (policy.Enabled === true) {
      doc = doc.ele(ServicePropertiesConstants.DEFAULT_RETAINED_VERSIONS_PER_BLOB_ELEMENT)
        .txt(1)
...
function parseDeleteRetentionPolicy(deleteRetentionPolicyXml) {
  ...
  if (typeof deleteRetentionPolicyXml.RetainedVersionsPerBlob !== 'undefined') {
    deleteRetentionPolicy.RetainedVersionsPerBlob = deleteRetentionPolicyXml.RetainedVersionsPerBlob;
  }
